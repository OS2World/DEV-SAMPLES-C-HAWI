/* $Header: D:\Projekte\Redaktion SE\DBase intim (2)\Source\RCS\DBASETST.C 1.1 1997/04/22 19:17:56 HAWI Exp $ */
...
INT main (INT argc,
   ...
   if   (argc == 2)
        ...
        if   ((rc = DBFCreate (&pdbf, argv [1], sizeof (dbfcreate) / sizeof (dbfcreate [0]), dbfcreate)) == RC_OK)
             ...
             fLogical = FALSE ;
             rc = DBFFLDPutLogical (pdbf, "LEDIG", &fLogical) ;
             rc = DBFMEMWrite (pdbf, "COMMENT", "Plattenmillionaer") ;
             rc = DBFRECWrite (pdbf, ulCount) ;
             rc = DBFRECAppend (pdbf, &ulCount) ;
